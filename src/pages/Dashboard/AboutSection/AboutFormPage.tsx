...
import { createAbout, getAbout, updateAbout, uploadImage } from "../../../api/about";
...
const ItemFormPage: React.FC = () => {
  ...
  const onSubmit = async (vals: FormValues) => {
    try {
      ...
      if (isEdit && id) {
        await updateAbout(Number(id), payload);
        alert("Updated");
      } else {
    ...
    <DashboardLayout>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-white p-6 rounded shadow max-w-2xl space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <button type="button" onClick={() => navigate("/dashboard-about")} className="px-4 py-2 bg-gray-200 rounded cursor-pointer">
            Cancel
          </button>
        </div>
      </form>
